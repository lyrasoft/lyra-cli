...
class AutoCompleteCommand implements CommandInterface
{
    ...
    {
        $home = $_SERVER['HOME'] ?? '~';
        $hookName = '.lyra_cli_completion_' . $shell;
    ...
    protected function hook(string $shell): string
    {
        return str_replace(
            '{name}',
            'lyra_cli_completion_' . uid(),
            file_get_contents(LYRA_ROOT . '/resources/shell/autocomplete/' . $shell)
        );
    }
    ...
    protected function genZshHook(): string
    {
        return str_replace(
            '{name}',
            'lyra_cli_completion_' . uid(),
            file_get_contents(LYRA_ROOT . '/resources/shell/autocomplete/zsh')
        );
    }
